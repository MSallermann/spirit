    ...
    else
    {
        for (int img = 0; img < Chain_Get_NOI(state.get()); ++img)
        {
            if (Simulation_Running_On_Image(state.get(), img))
            {
                ...
                int iteration=Simulation_Get_Iteration(state.get(), img);
                if (iteration - this->iteration_last > 100) {
...
void MainWindow::takeScreenshot(int iteration)
{
    //std::string tag = State_DateTime(state.get());
    
    //std::string name = tag + "_Screenshot_" + std::to_string(n_screenshots);
    std::string name = "_Screenshot_" + std::to_string(n_screenshots);
    ++n_screenshots;
...
void MainWindow::control_set_solver(std::string SolverName)
{
    this->controlWidget->setSolver(SolverName);
    Ui::MainWindow::statusBar->showMessage(tr(this->controlWidget->solverName().c_str()), 5000);
}
