...
const int WIDTH = 256; // Size of rendered mandelbrot set.
const int HEIGHT = 256; // Size of renderered mandelbrot set.
...
*/
		...
		private:
			...
			*/
			VkPipeline pipeline_all, pipeline_ddi;
			VkPipelineLayout pipelineLayout;
			...
			*/
			...
			VkBuffer buffer2;
			VkBuffer buffer3;
			VkDeviceMemory bufferMemory0;
			VkDeviceMemory bufferMemory1;
			VkDeviceMemory bufferMemory2;
			VkDeviceMemory bufferMemory3;
			uint32_t bufferSize0;
			uint32_t bufferSize1;
			uint32_t bufferSize2;
			uint32_t bufferSize3;
			...
			VulkanFFTPlan vulkanFFTPlan2 = { &context };
			/*
		...
		public:
			void init(regionbook regions_book, intfield regions) {
				// Buffer size of the storage buffer that will contain the rendered mandelbrot set.
				bufferSize0 = 3 * sizeof(float) * WIDTH * HEIGHT * DEPTH;
				bufferSize1 = 3 * sizeof(float) * WIDTH * HEIGHT * DEPTH;
				bufferSize2 = sizeof(Regionvalues) * 10;
				bufferSize3 = sizeof(int) * WIDTH * HEIGHT * DEPTH;
				// Initialize vulkan:

				createInstance();
				findPhysicalDevice();
				createDevice();
				createBuffer(&buffer0, &bufferMemory0, bufferSize0);
				createBuffer(&buffer1, &bufferMemory1, bufferSize1);
				createBuffer(&buffer2, &bufferMemory2, bufferSize2);
				createBuffer(&buffer3, &bufferMemory3, bufferSize3);
			...
			void readDataStream(VulkanFFTPlan* vulkanFFT, vectorfield& spins, vectorfield& imaginp, bool realinp, int index){
			...
			void writeDataStream(VulkanFFTPlan* vulkanFFT, vectorfield& gradient, vectorfield& imagoutp, int index) {
				VulkanFFTTransfer vulkanFFTTransfer;
				vulkanFFTTransfer.context = &context;
				vulkanFFTTransfer.size = vulkanFFTPlan.bufferSize;
					...
					uint32_t zOffset = vulkanFFTPlan.axes[0].sampleCount * vulkanFFTPlan.axes[1].sampleCount * z;
						for (uint32_t y = 0; y < vulkanFFTPlan.axes[1].sampleCount; ++y) {
							...
							for (uint32_t x = 0; x < vulkanFFTPlan.axes[0].sampleCount; ++x) {
								if (x + yzOffset == 16 + 32 * 256) {
									//std::cout << x + yzOffset << " " << std::real(data[x + yzOffset]) << " " << std::imag(data[x + yzOffset]) << " copy\n";
									std::cout << x + yzOffset << " " << gradient[x + yzOffset][index] << " " << imagoutp[x + yzOffset][index] << " b4\n";
								}
				...
				freeVulkanFFTTransfer(&vulkanFFTTransfer);
				std::cout << gradient[16+32*256][index] << " " << imagoutp[16 + 32 * 256][index] << " ss\n";
			}
			void initVulkanFFTContext(VulkanFFTContext* context) {
				...
				for (uint32_t i = 0; i < SUPPORTED_RADIX_LEVELS; ++i) {
					...
					uint32_t* code = readFile(filelength, "compfft.spv");
			...
			void createBufferFFT(VulkanFFTContext* context, VkBuffer* buffer, VkDeviceMemory* deviceMemory, VkBufferUsageFlags usage, VkMemoryPropertyFlags propertyFlags, VkDeviceSize size) {
				...
				assert(vkCreateBuffer(device, &bufferCreateInfo, NULL, buffer) == VK_SUCCESS);
				...
				assert(vkAllocateMemory(device, &memoryAllocateInfo, NULL, deviceMemory) == VK_SUCCESS);
			...
			void run(const vectorfield& spins, vectorfield& gradient) {
				...
				vectorfield real = vectorfield(256 * 256, { 0,0,0 });
				vectorfield imag = vectorfield(256 * 256, { 0,0,0 });
				//for (int index = 0; index < 3; index++) {
					for (int i = 0; i < 256 * 256; i++) {
						real[i] = { 1,0,0 };
					}
					readDataStream(&vulkanFFTPlan, real, imag,  true, 0);
					VkSubmitInfo submitInfo = {};
					submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
					submitInfo.commandBufferCount = 1;
					...
					//std::cout << spins[16 + 32 * 256][index] << " " << real[16 + 32 * 256][index] << " " << imag[16 + 32 * 256][index] << " 0\n";
					for (int i = 0; i < 256 * 256; i++) {
						std::cout << i << " " << real[i][0] << " " << imag[i][0] << " FFT\n";
						//std::cout << spins[16 + 32 * 256][index] << " " << real[16 + 32 * 256][index] << " " << imag[16 + 32 * 256][index] << " backward\n";
					}
					readDataStream(&vulkanFFTPlan, real, imag, false, 0);
					...
					writeDataStream(&vulkanFFTPlan, real, imag,0);
					for (int i = 0; i < 256*256; i++) {
						std::cout << i << " " << real[i][0] << " " << imag[i][0] << " revfft\n";
						//std::cout << spins[16 + 32 * 256][index] << " " << real[16 + 32 * 256][index] << " " << imag[16 + 32 * 256][index] << " backward\n";
					}
					//std::cout << spins[16 + 32 * 256][index] << " " << real[16 + 32 * 256][index] << " " << imag[16 + 32 * 256][index] << " backward\n";
...
/*
				std::cout << spins[16 + 32 * 256][0] << " " << real[16 + 32 * 256][0] << " " << imag[16 + 32 * 256][0] << " 0\n";
				std::cout << spins[16 + 32 * 256][1] << " " << real[16 + 32 * 256][1] << " " << imag[16 + 32 * 256][1] << " 1\n";
				std::cout << spins[16 + 32 * 256][2] << " " << real[16 + 32 * 256][2] << " " << imag[16 + 32 * 256][2] << " 2\n";
*/
				/*
				...
				std::cout << spins[32 + 32 * 256][0] << " " << gradient[32 + 32 * 256][0] << " 0\n";
				std::cout << spins[32 + 32 * 256][1] << " " << gradient[32 + 32 * 256][1] << " 1\n";
				std::cout << spins[32 + 32 * 256][2] << " " << gradient[32 + 32 * 256][2] << " 2\n";*/
			...
			void loadInput(regionbook regions_book, intfield regions) {
				...
				vkMapMemory(device, bufferMemory2, 0, bufferSize2, 0, &mappedMemory);
				...
				mappedMemory = NULL;
				// Map the buffer memory, so that we can read from it on the CPU.
				vkMapMemory(device, bufferMemory3, 0, bufferSize3, 0, &mappedMemory);
			...
			void loadData(const vectorfield& spins, vectorfield& gradient) {
				void* mappedMemory = NULL;
				// Map the buffer memory, so that we can read from it on the CPU.
				vkMapMemory(device, bufferMemory0, 0, bufferSize0, 0, &mappedMemory);
				Vector3* pmappedMemory0 = (Vector3*)mappedMemory;
				// Get the color data from the buffer, and cast it to bytes.
				// We save the data to a vector.
				//memcpy(pmappedMemory0, spins.data(), 3 * sizeof(float) * WIDTH * HEIGHT * DEPTH);
				// Done reading, so unmap.
				vkUnmapMemory(device, bufferMemory0);
				//std::cout << "0\n";
				mappedMemory = NULL;
				// Map the buffer memory, so that we can read from it on the CPU.
				vkMapMemory(device, bufferMemory1, 0, bufferSize1, 0, &mappedMemory);
			...
			void createDescriptorSet() {
				...
				*/
				...
				VkDescriptorBufferInfo descriptorBufferInfo0 = {};
				descriptorBufferInfo0.buffer = buffer0;
				descriptorBufferInfo0.offset = 0;
				descriptorBufferInfo0.range = bufferSize0;
				VkDescriptorBufferInfo descriptorBufferInfo1 = {};
				descriptorBufferInfo1.buffer = buffer1;
				descriptorBufferInfo1.offset = 0;
				descriptorBufferInfo1.range = bufferSize1;
				VkDescriptorBufferInfo descriptorBufferInfo2 = {};
				descriptorBufferInfo2.buffer = buffer2;
				descriptorBufferInfo2.offset = 0;
				descriptorBufferInfo2.range = bufferSize2;
				VkDescriptorBufferInfo descriptorBufferInfo3 = {};
				descriptorBufferInfo3.buffer = buffer3;
				descriptorBufferInfo3.offset = 0;
				descriptorBufferInfo3.range = bufferSize3;
			...
			void createCommandBuffer() {
				...
				*/
				...
				//vkCmdBindPipeline(commandBufferAll, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline_ddi);
			...
			void cleanup() {
				...
				*/
				...
				vkFreeMemory(device, bufferMemory0, NULL);
				vkFreeMemory(device, bufferMemory1, NULL);
				vkFreeMemory(device, bufferMemory2, NULL);
				vkFreeMemory(device, bufferMemory3, NULL);
				vkDestroyBuffer(device, buffer0, NULL);
				vkDestroyBuffer(device, buffer1, NULL);
				vkDestroyBuffer(device, buffer2, NULL);
				vkDestroyBuffer(device, buffer3, NULL);
