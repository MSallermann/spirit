...
const int WIDTH = 64; // Size of rendered mandelbrot set.
...
*/
		...
		public:
			...
			void recordVulkanFFT(VulkanFFTPlan* vulkanFFTPlan, VkCommandBuffer commandBuffer) {
				const uint32_t remap[3][3] = { {2, 0, 1}, {1, 2, 0},  {0, 1, 2}};
				//const uint32_t remap[3][3] = { {0, 1, 2} ,{1, 2, 0}, {2, 0, 1} };
				//const uint32_t remap[3][3] = { {2, 0, 1} ,{1, 2, 0}, {0, 1, 2}};
				for (uint32_t i = 0; i < COUNT_OF(vulkanFFTPlan->axes); ++i) {
					
					//i = 2 - i;
				//for (uint32_t i = COUNT_OF(vulkanFFTPlan->axes)-1; i >=0; i--) {
					if (vulkanFFTPlan->axes[remap[i][0]].sampleCount <= 1)
						continue;
					VulkanFFTAxis* vulkanFFTAxis = &vulkanFFTPlan->axes[remap[i][0]];
					std::cout << remap[i][0] << " " << vulkanFFTPlan->axes[remap[i][0]].sampleCount << "\n";
			...
			void planVulkanFFTAxis(VulkanFFTPlan* vulkanFFTPlan, uint32_t axis) {
...

				{
					...
					//const uint32_t remap[3][3] = { {1, 2, 0}, {2, 0, 1}, {0, 1, 2} };
					const uint32_t remap[3][3] = { {0, 1, 2}, {1, 2, 0}, {2, 0, 1} };
					//const uint32_t remap[3][3] = {  {1, 2, 0},{0, 1, 2}, {2, 0, 1} };
					//const uint32_t remap[3][3] = { {2, 0, 1} ,{1, 2, 0}, {0, 1, 2} };
					//const uint32_t remap[3][3] = {  {2, 0, 1} , {1, 2, 0}, {0, 1, 2} };
			...
			void run(const vectorfield& spins, vectorfield& gradient) {
				...
				vectorfield real = vectorfield(4*WIDTH * HEIGHT, { 0.0,0.0,0.0 });
				vectorfield imag = vectorfield(4 * WIDTH * HEIGHT, { 0.0,0.0,0.0 });
				for (int i = 0; i < WIDTH ; i++) {
					for (int j = 0; j <  HEIGHT; j++) {
						//real[i+j*2*WIDTH] = { 1.0,0.0,0.0 };
						real[i + j * 2 * WIDTH] = spins[i + j * WIDTH];
					}
				}
				//std::cout << spins[16 + 32 * 64][0] << " " << spins[16 + 32 * 64][1] << " " << spins[16 + 32 * 64][2] << "prefft\n";
				...
				for (int i = 1; i < WIDTH; i++) {
					for (int j = 0; j < 2 * HEIGHT; j++) {
						//real[i+j*2*WIDTH] = { 1.0,0.0,0.0 };
						//real[WIDTH + i + j * 2 * WIDTH] = real[WIDTH - i + j * 2 * WIDTH];
						//imag[WIDTH + i + j * 2 * WIDTH] = -imag[WIDTH - i + j * 2 * WIDTH];

					}
				}
				readDataStream(&vulkanFFTPlan, real, imag, false);
				//bufferTransfer(&context, vulkaniFFTPlan.buffer[0], vulkanFFTPlan.buffer[vulkanFFTPlan.resultInSwapBuffer], vulkanFFTPlan.bufferSize);
				bufferTransfer(&context, Spins_FFT, vulkanFFTPlan.buffer[vulkanFFTPlan.resultInSwapBuffer], bufferSizeSpins_FFT);
				...
				for (int i = 0; i < 2 * WIDTH; i++) {
					//std::cout << i << " " << real[i + 48 * 128][0] << " " << real[i + 48 * 128][1] << " " << real[i + 48 * 128][2] << " 48rconvovulkan\n";
					...
					//std::cout << i << " " << real[16 + i * 128][0] << " " << real[16 + i * 128][1] << " " << real[16 + i * 128][2] << " rconvovulkan\n";
				...
				for (int i = 1; i < WIDTH; i++) {
					for (int j = 0; j < 2 * HEIGHT; j++) {
						//real[i+j*2*WIDTH] = { 1.0,0.0,0.0 };
						//real[WIDTH + i + j * 2 * WIDTH] = real[WIDTH - i + (2*HEIGHT-j) * 2 * WIDTH];
						//imag[WIDTH + i + j * 2 * WIDTH] = imag[WIDTH - i + (2 * HEIGHT - j) * 2 * WIDTH];
				...
				for (int i = 0; i < int(WIDTH/2-1); i++) {
					for (int j = 0; j < 2*HEIGHT; j++) {
						//real[i+j*2*WIDTH] = { 1.0,0.0,0.0 };
						//real[WIDTH + 2*i + j * 2 * WIDTH+1] = real[WIDTH- 2 * i + j * 2 * WIDTH-1];
						//imag[WIDTH + 2 * i + j * 2 * WIDTH+1] = imag[WIDTH- 2 * i + j * 2 * WIDTH-1];
						//real[WIDTH + 2 * i + j * 2 * WIDTH+2] = -real[WIDTH - 2 * i + j * 2 * WIDTH-2];
						//imag[WIDTH + 2 * i + j * 2 * WIDTH+2] = -imag[WIDTH - 2 * i + j * 2 * WIDTH - 2];
						//gradient[i + j * WIDTH] -= 1e-7* 1e6 *real[i + j *2* WIDTH];
						//gradient[i + j * WIDTH] = real[i + j * 2 * WIDTH]-spins[i + j * WIDTH];
						//gradient[i + j * WIDTH] = real[i + j * 2 * WIDTH];
					}
					//std::cout << i << " " << gradient[i + 48 * 64][0] << " " << gradient[i + 48 * 64][1] << " " << gradient[i + 48 * 64][2] << " vulkan\n";
					//std::cout << i << " " << imag[i + 32 * 128][0] << " " << imag[i + 32 * 128][1] << " " << imag[i + 32 * 128][2] << " vulkan\n";
				}
				for (int i = 0; i < 2 * WIDTH; i++) {
					//std::cout << i << " " << real[i + 48 * 128][0] << " " << real[i + 48 * 128][1] << " " << real[i + 48 * 128][2] << " 48rconvovulkan\n";
					//std::cout << i << " " << real[i + 80 * 128][0] << " " << real[i + 80 * 128][1] << " " << real[i + 80 * 128][2] << " 80rconvovulkan\n";
				...
				for (int i = 0; i < 2 * WIDTH; i++) {
					//std::cout << i << " " << real[i + 48 * 128][0] << " " << real[i + 48 * 128][1] << " " << real[i + 48 * 128][2] << " rconvovulkan\n";
					//std::cout << i << " " << imag[i + 48 * 128][0] << " " << imag[i + 48* 128][1] << " " << imag[i + 48 * 128][2] << " 48iconvovulkan\n";
					//std::cout << i << " " << imag[i + 80 * 128][0] << " " << imag[i + 80 * 128][1] << " " << imag[i + 80 * 128][2] << " 80iconvovulkan\n";
					//std::cout << i << " " << real[16 + i * 128][0] << " " << real[16 + i * 128][1] << " " << real[16 + i * 128][2] << " rconvovulkan\n";
					//std::cout << i << " " << imag[16 + i * 128][0] << " " << imag[16 + i * 128][1] << " " << imag[16 + i * 128][2] << " iconvovulkan\n";

				}
				/*for (int i = 0; i < 2 * WIDTH; i++) {
					std::cout << i << " " << real[i + 48 * 128][0] << " " << real[i + 48 * 128][1] << " " << real[i + 48 * 128][2] << " convovulkan2\n";
				}*/
				commandBufferFFT = createCommandBufferFFT(&context, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
				recordVulkanFFT(&vulkaniFFTPlan, commandBufferFFT);
				vkEndCommandBuffer(commandBufferFFT);
				...
				for (int i = 0; i < WIDTH; i++) {
					...
					//std::cout << i << " " << gradient[i + 48 * 64][0] << " " << gradient[i + 48 * 64][1] << " " << gradient[i + 48 * 64][2] << " vulkan\n";
					//std::cout << i << " " << imag[i + 48 * 128][0] << " " << imag[i + 48 * 128][1] << " " << imag[i + 48 * 128][2] << " ivulkan\n";
				for (int i = 0; i < WIDTH; i++) {
						//std::cout << i << " " << gradient[i + 48 * 64][0] << " " << gradient[i + 48 * 64][1] << " " << gradient[i + 48 * 64][2] << " vulkan\n";
				...
				//std::cout << gradient[16 + 32 * 64][0] << " " << gradient[16 + 32 * 64][1] << " " << gradient[16 + 32 * 64][2] << "vulkan\n";
				/*for (int i = 0; i < WIDTH; i++) {
					for (int j = 0; j < HEIGHT; j++) {
						//std::cout << i << " " << real[i + j * 2 * WIDTH][0] << " " << imag[i + j * 2 * WIDTH][0] << " 0 ifft\n";
						//gradient[i+j*WIDTH]=
					}
				}
				for (int i = 0; i < WIDTH * HEIGHT; i++) {
					//std::cout << i << " " << real[i][0] << " " << imag[i][0] << " 0 ifft\n";
					//std::cout << i << " " << real[i][1] << " " << imag[i][1] << " 1 fft\n";
					//std::cout << i << " " << real[i][2] << " " << imag[i][2] << " 2 fft\n";
					//std::cout << spins[16 + 32 * 64][index] << " " << real[16 + 32 * 64][index] << " " << imag[16 + 32 * 64][index] << " backward\n";
				}*/
			...
			void createCommandBufferConvolution() {
				...
				*/
				...
				vkCmdDispatch(commandBufferConvolution, (uint32_t)ceil(2 * WIDTH / float(WORKGROUP_SIZE)), (uint32_t)ceil(2 * HEIGHT / float(WORKGROUP_SIZE)), (uint32_t)DEPTH);
