	...
	public:
		...
		void createComputeGradients_noDDI(VulkanCollection* collection, VulkanFFTPlan* vulkanFFTPlan, uint32_t save_energy) {
...

			{
				...
				if (launchConfiguration.double_precision_gradient == true)
					code = readFile(filelength, "shaders/gradient_double.spv");
				else
					code = readFile(filelength, "shaders/gradient_float.spv");
				VkShaderModuleCreateInfo createInfo = {};
				createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
				createInfo.pCode = code;
				createInfo.codeSize = filelength;
		...
		void initReduceBuffers(VulkanReduce * vulkanReduce) {
			...
			for (int i = 0; i < vulkanReduce->bufferNum ; i++)
			{
				...
				createBufferFFT(vulkanReduce->context, &vulkanReduce->buffer[i], &vulkanReduce->deviceMemory[i], VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_HEAP_DEVICE_LOCAL_BIT, 9*vulkanReduce->sizes[i] * sizeof(float));
			}
		...
		void createLBFGS(VulkanLBFGS* vulkanLBFGS) {
			...
			void* map;
			vkMapMemory(device, vulkanReduce.deviceMemoryLastMax, 9 * sizeof(scalar), 2*sizeof(scalar), 0, &map);
		...
		void createVP(VulkanLBFGS* vulkanLBFGS) {
			...
			//createOsoCalcGradients(&vulkanLBFGS->collectionOsoCalcGradients, 0);

			vulkanLBFGS->applyVP1Consts.dt = launchConfiguration.gamma/ 0.176085964411;
			vulkanLBFGS->applyVP1Consts.dt_div_m = 0.5*vulkanLBFGS->applyVP1Consts.dt /0.01;
			...
			vulkanFFTTransfer.size = vulkanLBFGS->bufferSizes[0];
			vulkanFFTTransfer.deviceBuffer = vulkanLBFGS->buffer[0];

			scalar* data_velocity = (scalar*)createVulkanFFTUpload(&vulkanFFTTransfer);
			for (uint32_t i = 0; i < 3 * SIZES[0] * SIZES[1]* SIZES[2]; ++i) {
				data_velocity[i] = 0.0;
			}
			freeVulkanFFTTransfer(&vulkanFFTTransfer);
			vulkanFFTTransfer.size = vulkanLBFGS->bufferSizes[1];
			vulkanFFTTransfer.deviceBuffer = vulkanLBFGS->buffer[1];

			data_velocity = (scalar*)createVulkanFFTUpload(&vulkanFFTTransfer);
			for (uint32_t i = 0; i < 3 * SIZES[0] * SIZES[1] * SIZES[2]; ++i) {
				data_velocity[i] = 0.0;
			}
			freeVulkanFFTTransfer(&vulkanFFTTransfer);
			...
			*/
			...
			vulkanLBFGS->ReduceDotConsts[3] = SIZES[0] * SIZES[1];
			//createReduceEnergy(&collectionReduceDotEnergy, &bufferGradient, bufferSizeGradient, &bufferSpins, bufferSizeSpins);
			createReduceEnergyFinish(&vulkanLBFGS->collectionReduceEnergyFinish, &vulkanReduce, 9);
		...
		void createApplyVP1(VulkanCollection* collection) {
...

			{
				...
				uint32_t* code;
				if (launchConfiguration.double_precision_rotate == true)
					code = readFile(filelength, "shaders/ApplyVP1_double.spv");
				else
					code = readFile(filelength, "shaders/ApplyVP1_float.spv");
