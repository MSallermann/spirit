...
namespace VulkanCompute
{
	...
	public:
		...
		void create() {}
		void init(regionbook regions_book, intfield regions, field<Engine::FFT::FFT_cpx_type> transformed_dipole_matrices, std::shared_ptr<Data::Geometry> geometry) {
			...
			vulkanFFTTransferGradient.deviceBuffer = bufferGradient;
			...
			vulkanFFTTransferSpins.deviceBuffer = bufferSpins;
		...
		void initVulkanFFTContext(VulkanFFTContext* context) {
			...
			char filenames[3][33];
			sprintf(filenames[0], "shaders/radix_fft/fft_radix2.spv");
		...
		void createVulkaniFFT(VulkanFFTPlan* vulkaniFFTPlan, VulkanFFTPlan* vulkanFFTPlan) {
			...
			for (uint32_t i = 0; i < COUNT_OF(vulkaniFFTPlan->axes); ++i) {
				if (vulkaniFFTPlan->axes[remap[i]].sampleCount > 1)
		...
		void recordVulkanFFT(VulkanFFTPlan* vulkanFFTPlan, VkCommandBuffer commandBuffer) {
			...
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[0][1]] , (uint32_t)512);
			LOCAL_SIZE_FFT[2] = 1;
			for (uint32_t j = 0; j < vulkanFFTAxis->stageCount; ++j) {
				...
				vkCmdDispatch(commandBuffer, (uint32_t)ceil(vulkanFFTAxis->sampleCount / vulkanFFTAxis->stageRadix[j] / ((scalar)LOCAL_SIZE_FFT[0])), (uint32_t)ceil((vulkanFFTPlan->axes[remap[0][1]].sampleCount / 2 ) / ((scalar)LOCAL_SIZE_FFT[1])), vulkanFFTPlan->axes[remap[0][2]].sampleCount);
				bit_swap = !bit_swap;
			}
			...
			vkCmdDispatch(commandBuffer, (uint32_t)ceil((vulkanFFTPlan->axes[0].sampleCount / 2 + 1) / ((scalar)LOCAL_SIZE[0])), (uint32_t)ceil(vulkanFFTPlan->axes[1].sampleCount / 2 / ((scalar)LOCAL_SIZE[1])), (uint32_t)ceil(vulkanFFTPlan->axes[2].sampleCount / ((scalar)LOCAL_SIZE[2])));

			vulkanFFTAxis = &vulkanFFTPlan->axes[remap[1][0]];
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[1][1]] + 1, (uint32_t)513);
			LOCAL_SIZE_FFT[2] = 1;
			for (uint32_t j = 0; j < vulkanFFTAxis->stageCount; ++j) {
				uint32_t dynamicOffset = vulkanFFTPlan->context->uboAlignment * j;
				vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, vulkanFFTAxis->pipelines[30 - __builtin_clz(vulkanFFTAxis->stageRadix[j])]);
				...
				vkCmdDispatch(commandBuffer, (uint32_t)ceil(vulkanFFTAxis->sampleCount / vulkanFFTAxis->stageRadix[j] / ((scalar)LOCAL_SIZE_FFT[0])), (uint32_t)ceil((vulkanFFTPlan->axes[remap[1][1]].sampleCount / 2 + 1) / ((scalar)LOCAL_SIZE_FFT[1])), vulkanFFTPlan->axes[remap[1][2]].sampleCount);
				bit_swap = !bit_swap;
			}
			vulkanFFTPlan->resultInSwapBuffer = bit_swap;
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[2][1]] + 1, (uint32_t)513);
			LOCAL_SIZE_FFT[2] = 1;
			if (vulkanFFTPlan->axes[remap[2][0]].sampleCount > 1) {
				...
				for (uint32_t j = 0; j < vulkanFFTAxis->stageCount; ++j) {
					...
					vkCmdDispatch(commandBuffer, (uint32_t)ceil(vulkanFFTAxis->sampleCount / vulkanFFTAxis->stageRadix[j] / ((scalar)LOCAL_SIZE_FFT[0])), (uint32_t)ceil((vulkanFFTPlan->axes[remap[2][1]].sampleCount / 2 + 1) / ((scalar)LOCAL_SIZE_FFT[1])), vulkanFFTPlan->axes[remap[2][2]].sampleCount);
					bit_swap = !bit_swap;
				}
		...
		void recordVulkaniFFT(VulkanFFTPlan* vulkanFFTPlan, VkCommandBuffer commandBuffer) {
			...
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[0][1]] + 1, (uint32_t)513);
			LOCAL_SIZE_FFT[2] = 1;
			if (vulkanFFTPlan->axes[remap[0][0]].sampleCount > 1) {
				...
				for (uint32_t j = 0; j < vulkanFFTAxis->stageCount; ++j) {
					...
					vkCmdDispatch(commandBuffer, (uint32_t)ceil(vulkanFFTAxis->sampleCount / vulkanFFTAxis->stageRadix[j] / ((scalar)LOCAL_SIZE_FFT[0])), (uint32_t)ceil((vulkanFFTPlan->axes[remap[0][1]].sampleCount / 2 + 1) / ((scalar)LOCAL_SIZE_FFT[1])), vulkanFFTPlan->axes[remap[0][2]].sampleCount);
					bit_swap = !bit_swap;
				}
			...
			vulkanFFTAxis = &vulkanFFTPlan->axes[remap[2][0]];
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[2][1]] , (uint32_t)512);
			LOCAL_SIZE_FFT[2] = 1;
			for (uint32_t j = 0; j < vulkanFFTAxis->stageCount; ++j) {
				uint32_t dynamicOffset = vulkanFFTPlan->context->uboAlignment * j;
				vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, vulkanFFTAxis->pipelines[30 - __builtin_clz(vulkanFFTAxis->stageRadix[j])]);
				...
				vkCmdDispatch(commandBuffer, (uint32_t)ceil(vulkanFFTAxis->sampleCount / vulkanFFTAxis->stageRadix[j] / ((scalar)LOCAL_SIZE_FFT[0])), (uint32_t)ceil((vulkanFFTPlan->axes[remap[2][1]].sampleCount / 2) / ((scalar)LOCAL_SIZE_FFT[1])), vulkanFFTPlan->axes[remap[2][2]].sampleCount);
				bit_swap = !bit_swap;
			}
			//}
		...
		void planVulkanFFTAxis(VulkanFFTPlan* vulkanFFTPlan, uint32_t axis) {
...

			{
				...
				LOCAL_SIZE_FFT[0] = 1;
				LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[axis][1]] / 2 + 1, (uint32_t)513);
				LOCAL_SIZE_FFT[2] = 1;
				...
				specializationInfo.pData = &LOCAL_SIZE_FFT;
		...
		void createConvolution(VulkanCollection* collection, VulkanFFTPlan* vulkanFFTPlan)
		{
...

			{
				...
				specializationInfo.mapEntryCount = static_cast<uint32_t>(specializationMapEntries.size());
				specializationInfo.pMapEntries = specializationMapEntries.data();
				specializationInfo.pData = &LOCAL_SIZE;
			...
			}
			{
				...
				vkCmdBindDescriptorSets(collection[0].commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, collection[0].pipelineLayout, 0, 1, &collection[0].descriptorSets[0], 0, NULL);
				vkCmdBindPipeline(collection[0].commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, collection[0].pipelines[0]);
				vkCmdDispatch(collection[0].commandBuffer, (uint32_t)ceil((vulkanFFTPlan->axes[0].sampleCount / 2 + 1) / ((scalar)LOCAL_SIZE[0])), (uint32_t)ceil(vulkanFFTPlan->axes[1].sampleCount / ((scalar)LOCAL_SIZE[1])), (uint32_t)ceil(vulkanFFTPlan->axes[2].sampleCount / ((scalar)LOCAL_SIZE[2])));
				VK_CHECK_RESULT(vkEndCommandBuffer(collection[0].commandBuffer)); // end recording commands.
			}
		}
		...
		void createFillZero(VulkanCollection* collection, VulkanFFTPlan* vulkanFFTPlan) {
...

			{
				...
				VkPipelineShaderStageCreateInfo pipelineShaderStageCreateInfo = { };
				VkComputePipelineCreateInfo computePipelineCreateInfo = { };
				uint32_t zoffset = 3 * vulkanFFTPlan->axes[0].sampleCount * vulkanFFTPlan->axes[1].sampleCount;
		...
		void createReduce(VulkanCollection* collection) {
...

			{
				...
				specializationData.sumSubGroupSize = 64;
		...
		void createReduceDot(VulkanCollection* collection, VkBuffer* input1, VkDeviceSize input1_size, VkBuffer* input2, VkDeviceSize input2_size) {
			...
			beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
			VK_CHECK_RESULT(vkBeginCommandBuffer(collection[0].commandBuffer, &beginInfo)); // start recording commands.
			vkCmdPushConstants(collection[0].commandBuffer, collection[0].pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, 3 * sizeof(uint32_t), vulkanLBFGS.ReduceDotConsts);
			vkCmdBindPipeline(collection[0].commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, collection[0].pipelines[0]);
			vkCmdBindDescriptorSets(collection[0].commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, collection[0].pipelineLayout, 0, 1, &collection[0].descriptorSets[0], 0, NULL);
			vkCmdDispatch(collection[0].commandBuffer, vulkanReduce.sizes[0], 1, 1);
			VK_CHECK_RESULT(vkEndCommandBuffer(collection[0].commandBuffer)); // end recording commands.
		...
		void createOsoCalcGradients(VulkanCollection* collection) {
			{
				VkDescriptorPoolSize descriptorPoolSize[1] = { };
				descriptorPoolSize[0].type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
				descriptorPoolSize[0].descriptorCount = 3;
		...
		void createOsoRotate(VulkanCollection* collection) {
			{
				VkDescriptorPoolSize descriptorPoolSize[1] = { };
				descriptorPoolSize[0].type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
				descriptorPoolSize[0].descriptorCount = 2;
		...
		void createLBFGS(VulkanLBFGS* vulkanLBFGS) {
			...
			createOsoCalcGradients(&vulkanLBFGS->collectionOsoCalcGradients);
			createOsoRotate(&vulkanLBFGS->collectionOsoRotate);
