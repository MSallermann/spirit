...
namespace VulkanCompute
{
	...
	public:
		...
		void create() {}
		void init(regionbook regions_book, intfield regions, field<Engine::FFT::FFT_cpx_type> transformed_dipole_matrices, std::shared_ptr<Data::Geometry> geometry, int solver_type) {
		...
		void recordVulkanFFT(VulkanFFTPlan* vulkanFFTPlan, VkCommandBuffer commandBuffer) {
			...
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[0][1]] / 2, (uint32_t)512);
			LOCAL_SIZE_FFT[2] = 1;
			...
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[1][1]] / 2 + 1, (uint32_t)513);
			LOCAL_SIZE_FFT[2] = 1;
			for (uint32_t j = 0; j < vulkanFFTAxis->stageCount; ++j) {
				uint32_t dynamicOffset = vulkanFFTPlan->context->uboAlignment * j;
				vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, vulkanFFTAxis->pipelines[30 - __builtin_clz(vulkanFFTAxis->stageRadix[j])]);
			...
			vulkanFFTPlan->resultInSwapBuffer = bit_swap;
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[2][1]] / 2 + 1, (uint32_t)513);
			LOCAL_SIZE_FFT[2] = 1;
		...
		void recordVulkaniFFT(VulkanFFTPlan* vulkanFFTPlan, VkCommandBuffer commandBuffer) {
			...
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[0][1]] / 2 + 1, (uint32_t)513);
			LOCAL_SIZE_FFT[2] = 1;
			...
			vulkanFFTAxis = &vulkanFFTPlan->axes[remap[2][0]];
			LOCAL_SIZE_FFT[0] = 1;
			LOCAL_SIZE_FFT[1] = std::min(SIZES[remap[2][1]] / 2, (uint32_t)512);
			LOCAL_SIZE_FFT[2] = 1;
			for (uint32_t j = 0; j < vulkanFFTAxis->stageCount; ++j) {
				uint32_t dynamicOffset = vulkanFFTPlan->context->uboAlignment * j;
				vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, vulkanFFTAxis->pipelines[30 - __builtin_clz(vulkanFFTAxis->stageRadix[j])]);
		...
		void createFillZero(VulkanCollection* collection, VulkanFFTPlan* vulkanFFTPlan) {
			...
			}
			{
				...
				vkCmdBindDescriptorSets(collection[0].commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, collection[0].pipelineLayout, 0, 1, &collection[0].descriptorSets[0], 0, NULL);
				vkCmdBindPipeline(collection[0].commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, collection[0].pipelines[0]);
				vkCmdDispatch(collection[0].commandBuffer, (uint32_t)ceil(3 * vulkanFFTPlan->axes[0].sampleCount * (vulkanFFTPlan->axes[1].sampleCount) / 1024.0f), vulkanFFTPlan->axes[2].sampleCount, 1);
				VK_CHECK_RESULT(vkEndCommandBuffer(collection[0].commandBuffer)); // end recording commands.
			}
		}
