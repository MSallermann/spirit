...
namespace VulkanCompute
{
	...
	public:
		...
		void createVulkanFFT(VulkanFFTPlan* vulkanFFTPlan) {
			...
			for (uint32_t i = 0; i < COUNT_OF(vulkanFFTPlan->axes); ++i) {
				if (vulkanFFTPlan->axes[i].sampleCount > 1)
					planVulkanFFTAxis(vulkanFFTPlan, i, &uboFrame[0][i]);
		...
		void createVulkaniFFT(VulkanFFTPlan* vulkaniFFTPlan, VulkanFFTPlan* vulkanFFTPlan) {
			...
			for (uint32_t i = 0; i < COUNT_OF(vulkaniFFTPlan->axes); ++i) {
				if (vulkaniFFTPlan->axes[i].sampleCount > 1)
					planVulkanFFTAxis(vulkaniFFTPlan, i, &uboFrame[1][i]);
			}
		}
		...
		void planVulkanFFTAxis(VulkanFFTPlan* vulkanFFTPlan, uint32_t axis, VulkanFFTUBO *uboFrame) {
			const uint32_t remap[3][3] = { {0, 1, 2}, {1, 0, 2}, {2, 0, 1} };
			VulkanFFTAxis* vulkanFFTAxis = &vulkanFFTPlan->axes[axis];
			{
				...
				vulkanFFTAxis->uboSize = vulkanFFTPlan->context->uboAlignment;
...

			{
				...
				for (uint32_t i = 0; i < SUPPORTED_RADIX_LEVELS; ++i) {
					...
					if (axis == 0) {
						if (vulkanFFTPlan->inverse == true) {
							pipelineShaderStageCreateInfo[i].module = vulkanFFTPlan->context->shaderModules[1];
						}
						else {
							pipelineShaderStageCreateInfo[i].module = vulkanFFTPlan->context->shaderModules[0];
						}
					}
					else {
						pipelineShaderStageCreateInfo[i].module = vulkanFFTPlan->context->shaderModules[2];
					}
		...
		void createReduceDotFinish(VulkanCollection* collection, VulkanReduce * vulkanReduce) {
			{
				VkDescriptorPoolSize descriptorPoolSize[1] = { };
				descriptorPoolSize[0].type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
		...
		void createApplyVP1(VulkanCollection* collection) {
...

			{
				...
				for (uint32_t i = 0; i < COUNT_OF(descriptorType); ++i) {
					...
					if (i == 3) {
						descriptorBufferInfo.buffer = vulkanReduce2.buffer[0];
						descriptorBufferInfo.offset = 0;
						descriptorBufferInfo.range = vulkanReduce2.sizes[0] * sizeof(float);
					}
		...
		void createApplyVP2(VulkanCollection* collection) {
...

			{
				...
				for (uint32_t i = 0; i < COUNT_OF(descriptorType); ++i) {
					...
					if (i == 3) {
						descriptorBufferInfo.buffer = vulkanReduce.buffer[vulkanReduce2.bufferNum - 1];
						descriptorBufferInfo.offset = 0;
						descriptorBufferInfo.range = vulkanReduce.sizes[vulkanReduce2.bufferNum - 1] * sizeof(float);
					}
					if (i == 4) {
						descriptorBufferInfo.buffer = vulkanReduce2.buffer[vulkanReduce2.bufferNum - 1];
						descriptorBufferInfo.offset = 0;
						descriptorBufferInfo.range = vulkanReduce2.sizes[vulkanReduce2.bufferNum - 1] * sizeof(float);
					}
					VkWriteDescriptorSet writeDescriptorSet = { };
					writeDescriptorSet.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
					writeDescriptorSet.dstSet = collection[0].descriptorSets[0];
		...
		void vp_get_searchdir(scalar m_temp_inv, scalar dt, uint32_t iteration) {
			int nos = SIZES[0] * SIZES[1] * SIZES[2];
			auto time0 = std::chrono::steady_clock::now();
			runCommandBuffer(vulkanLBFGS.collectionApply1.commandBuffer);
			auto time1 = std::chrono::steady_clock::now();
			...
			if (iteration == 0) {
				...
				vulkanLBFGS.applyVP2Consts.m_temp_inv = m_temp_inv;
		...
		void createLBFGS(VulkanLBFGS* vulkanLBFGS) {
			...
			createReduceDotFinish(&vulkanLBFGS->collectionReduceDotFinish, &vulkanReduce);
