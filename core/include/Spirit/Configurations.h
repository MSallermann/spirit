...
PREFIX void Configuration_APStripe(State* state, const float direction[3]= defaultPos, const float position[3] = defaultPos, const float r_cut_rectangular[3] = defaultRect, float r_cut_cylindrical = -1, float r_cut_spherical = -1, bool inverted = false, int idx_image = -1, int idx_chain = -1) SUFFIX;
...
// Create a skyrmion configuration
PREFIX void Configuration_Vortex(State* state, float r, float order, float phase, bool upDown, bool achiral, bool rl, const float position[3] = defaultPos, const float r_cut_rectangular[3] = defaultRect, float r_cut_cylindrical = -1, float r_cut_spherical = -1, bool inverted = false, int idx_image = -1, int idx_chain = -1) SUFFIX;
