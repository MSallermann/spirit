...
void Configuration_APStripe(State* state, const float direction[3], const float position[3],
    const float r_cut_rectangular[3], float r_cut_cylindrical,
    float r_cut_spherical, bool inverted, int idx_image, int idx_chain) noexcept
    try
{
    ...
    Vector3 vdir{ direction[0], direction[1], direction[2] };
    image->Lock();
    Utility::Configurations::APStripe(*image, vdir, filter);
...
void Configuration_Vortex(State* state, float r, float order, float phase, bool upDown,
    bool achiral, bool rl, const float position[3],
    const float r_cut_rectangular[3], float r_cut_cylindrical,
    float r_cut_spherical, bool inverted, int idx_image, int idx_chain) noexcept
    try
{
    ...
    // Apply configuration
    image->Lock();
    Utility::Configurations::Vortex(*image, vpos, r, order, phase, upDown, achiral, rl,
        false, filter);
    image->geometry->Apply_Pinning(*image->spins);
    image->Unlock();
    ...
    if (rl != 0) parameterstring += fmt::format(", rl={}", rl);
    Log(Utility::Log_Level::Info, Utility::Log_Sender::API,
        "Set vortex configuration, " + parameterstring + ". " + filterstring,
        idx_image, idx_chain);
}
catch (...)
{
