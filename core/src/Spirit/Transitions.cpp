...
void Transition_Without_Zero_Modes( State * state, int idx_1, int idx_2, int idx_chain ) noexcept
try
{
    int idx_image = -1;
    std::shared_ptr<Data::Spin_System> image;
    ...
    if( idx_2 <= idx_1 )
    {
        Log( Utility::Log_Level::Error, Utility::Log_Sender::API,
             fmt::format(
                 "Cannot set transiton without zero modes between images {} and {}, because the second index needs to be "
                 "larger than the first",
                 idx_1 + 1, idx_2 + 1 ),
             -1, idx_chain );
        return;
    ...
    try
    {
        Utility::Configuration_Chain::Transition_Without_Zero_Modes( chain, idx_1, idx_2 );
        for( int img = 0; img < chain->noi; ++img )
            chain->images[img]->geometry->Apply_Pinning( *chain->images[img]->spins );

        Log( Utility::Log_Level::Info, Utility::Log_Sender::API,
             fmt::format( "Set transition without zero modes between images {} and {}", idx_1 + 1, idx_2 + 1 ), -1,
             idx_chain );
    }
    catch( ... )
    {
