...
try
{
    ...
    try
    {
        ...
        vectorfield test=vectorfield(256*256*64, Vector3{ 0,0,0 });;
        int shift = 64;
        int transl = 16;
        scalar rad = 70;
        for (int i = 0; i < 128; i++) {
            for (int j = 0; j < 128; j++) {
                for (int k = 0; k < 64; k++) {
                    if (((i - 64 ) * (i - 64 ) + (j - 64 ) * (j - 64) < rad * rad))
                        test[i +transl+ (j+transl) * 256 + k * 256 * 256] = spins [i+ shift + (j+ shift) * 256 + k * 256 * 256];
                }
            }
        }
        ...
        for (int i = 0; i < 128; i++) {
            for (int j = 0; j < 128; j++) {
                for (int k = 0; k < 64; k++) {
                    ...
                    if (((i-64)* (i - 64 )+ (j - 64)* (j - 64 )<rad*rad) )
                        test[256 - i - transl + (256 - j - transl) * 256 + k * 256 * 256] =  spins[i + shift + (j + shift) * 256 + k * 256 * 256];

                        //if ((test[256 - i + (256 - j) * 256 + k * 256 * 256][3] < 0.9) && (spins[i + 48 + (j + 48) * 256 + k * 256 * 256][3] < 0.9))
                            //test[256-i-transl + (256-j-transl) * 256 + k * 256 * 256] = (test[256-i + (256-j) * 256 + k * 256 * 256] +spins[i + shift + (j + shift) * 256 + k * 256 * 256]).normalized();
                }
            }
        }
        for (int i = 0; i < 256; i++) {
            for (int j = 0; j < 256; j++) {
                for (int k = 0; k < 64; k++) {
                    spins[i + j * 256 + k * 256 * 256] = test[i + j * 256 + k * 256 * 256];
                }
            }
        }*/
