...
namespace Engine
{
		...
		cell_sizes(cell_sizes), Ms(Ms), region_num(region_num)
	{
		...
		for (int i = 0; i < ((int)(geometry->nos)); i++) {
			...
			if (pow((x - geometry->n_cells[0] / 2), 2) + pow((y - geometry->n_cells[1] / 2), 2) > 0.2f * geometry->n_cells[0] * geometry->n_cells[0])
			{
				//std::cout << x << " " << y << " " << z << "\n";
				//regions[i] = 1;
			}
		...
		for (int i=0;i<region_num;i++){
			...
			test.cell_sizes_inv[0] = 1/cell_sizes[0];
			test.cell_sizes_inv[1] = 1 / cell_sizes[1];
			test.cell_sizes_inv[2] = 1 / cell_sizes[2];
	...
	void Hamiltonian_Micromagnetic::init_vulkan(VulkanCompute::ComputeApplication* app) {
		if (geometry->n_cells[2] == 1) {
			...
			field <float> transformed_dipole_matrices_cut = field <float>(2 * 6 * 2 * geometry->n_cells[1] * (geometry->n_cells[0] + 1), { 0 });
			for (int j = 0; j < 2 * geometry->n_cells[1]; j++) {
				for (int i = 0; i < geometry->n_cells[0] + 1; i++) {
					for (int k = 0; k < 6; k++) {
						transformed_dipole_matrices_cut[ (j + i * 2 * (geometry->n_cells[1])) + k* npad_kernel] = transformed_dipole_matrices[6 * (i + j * 2 * geometry->n_cells[0]) + k][0];
						transformed_dipole_matrices_cut[(j + i * 2 * (geometry->n_cells[1])) + k * npad_kernel+npad_kernel*6] = transformed_dipole_matrices[6 * (i + j * 2 * geometry->n_cells[0]) + k][1];
			...
			app[0].init(regions_book, regions, transformed_dipole_matrices_cut, geometry, 1 );
		}
		if (geometry->n_cells[2] > 1) {
			...
			int npad_kernel = 4 * geometry->n_cells[2]  * geometry->n_cells[1] * (geometry->n_cells[0] + 1);

			field <float> transformed_dipole_matrices_cut = field <float>(2*6 * 4 * geometry->n_cells[2] * geometry->n_cells[1] * (geometry->n_cells[0] + 1), { 0});
			for (int l = 0; l < 2 * geometry->n_cells[2]; l++) {
				for (int j = 0; j < 2 * geometry->n_cells[1]; j++) {
					for (int i = 0; i < geometry->n_cells[0] + 1; i++) {
						for (int k = 0; k < 6; k++) {
							transformed_dipole_matrices_cut[(j + i * 2 * (geometry->n_cells[1]) + 6 * l * (geometry->n_cells[0] + 1) * 2 * (geometry->n_cells[1])) + k * npad_kernel] = transformed_dipole_matrices[6 * (i + j * 2 * geometry->n_cells[0] + l * 4 * (geometry->n_cells[0]) * (geometry->n_cells[1])) + k][0];
							transformed_dipole_matrices_cut[(j + i * 2 * (geometry->n_cells[1]) + 6 * l * (geometry->n_cells[0] + 1) * 2 * (geometry->n_cells[1])) + k * npad_kernel + npad_kernel * 6] = transformed_dipole_matrices[6 * (i + j * 2 * geometry->n_cells[0] + l * 4 * (geometry->n_cells[0]) * (geometry->n_cells[1])) + k][1];
			...
			/*for (int l = 0; l < geometry->n_cells[2]; l++) {
				for (int j = 0; j < 2 * geometry->n_cells[1]; j++) {
					for (int i = 0; i < geometry->n_cells[0] + 1; i++) {
						for (int k = 0; k < 6; k++) {
							transformed_dipole_matrices_cut[6 * (n2/2+i + j * (geometry->n_cells[0] + 1) + l * (geometry->n_cells[0] + 1) * 2 * (geometry->n_cells[1])) + k] = transformed_dipole_matrices[6 * ( i + j * 2 * geometry->n_cells[0] + l * 4 * (geometry->n_cells[0]) * (geometry->n_cells[1])) + k];
						}
						//std::cout << transformed_dipole_matrices_cut[6 * (i + j * (geometry->n_cells[0] + 1) + l * (geometry->n_cells[0] + 1) * 2 * (geometry->n_cells[1])) + 0][0] << " " << transformed_dipole_matrices_cut[6 * (i + j * (geometry->n_cells[0] + 1) + l * (geometry->n_cells[0] + 1) * 2 * (geometry->n_cells[1])) + 0][1] << "\n";
