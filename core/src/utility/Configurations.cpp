...
namespace Utility
{
    namespace Configurations
    {
        ...
        void APStripe(Data::Spin_System& s, Vector3 v, filterfunction filter)
        {
            if (v.norm() < 1e-8)
            {
                Log(Log_Level::Warning, Log_Sender::All, fmt::format("Homogeneous vector was ({}, {}, {}) and got set to (0, 0, 1)", v[0], v[1], v[2]));
            ...
            for (int iatom = 0; iatom < s.nos/2; ++iatom)
            {

                spins[iatom] = { 1,0,0 };
            }
            for (int iatom = s.nos / 2; iatom < s.nos ; ++iatom)
            {
            ...
            if (r != 0.0)
            {
                ...
                for (int n = 0; n<s.nos; n++)
                {
                    ...
                    if (filter(spins[n], positions[n]))
                    {
                        ...
                        d = sqrt((positions[n][0] - pos[0])* (positions[n][0] - pos[0])+ (positions[n][1] - pos[1])* (positions[n][1] - pos[1])+ z_scale * z_scale *(positions[n][2] - pos[2])* (positions[n][2] - pos[2]));
                        // Theta
                        if (d == 0)
                        {
                            T = 0;
                        ...
                        {
                            T = z_scale *(positions[n][2] - pos[2]) / d; // angle with respect to the main axis of toroid [0,0,1]
                        }
                        T = acos(T);
                        // ...
                        ...
                        // Spin orientation
                        spins[n][0] = sin(t)*cos(order * f+ Pi/2);
                        spins[n][1] = sin(t)*sin(order * f+Pi/2);
                        spins[n][2] = cos(t);
                    }
                }
            }
...
/*
        ...
        // end Skyrmion
        void Vortex(Data::Spin_System& s, Vector3 pos, scalar r, scalar order, scalar phase, bool upDown, bool achiral, bool rl, bool experimental, filterfunction filter)
        {
...
/*
            ...
            for (iatom = 0; iatom < s.nos; ++iatom)
            {
                ...
                if (filter(spins[iatom], positions[iatom]))
                {
                    ...
                    spins[iatom][0] = ksi * std::cos(phi_i);
                    spins[iatom][1] = ksi * std::sin(phi_i);
                    spins[iatom][2] = 0;
                }
            }
            for (auto& v : spins) v.normalize();
